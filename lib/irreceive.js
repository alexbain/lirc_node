 ...
 * @param  {Function} callback Function to execute when the key / remote combo
 *                             is detected
 * @param  {Int}      throttle Optional. Omit for no throttle. If provided,
 *                             read as "throttle input to one every X milliseconds".
 *                             If set to 0 this will disable repeat callbacks.
...
IRReceive.prototype.addListener = function () {
  ...
  function parameterError() {
    throw new Error('lirc_node: IRReceive#addListener: bad parameters. ' +
        'Please use addListener([key,[ remote,]] callback[, throttle]);');
  }
...
IRReceive.prototype._handleIRData = function (data) {
  this.callbacks.forEach(function (callback) {
    ...
    if ((callback.throttle !== 0 || parseInt(d.repeat, 10) === 0) &&
